    ...
    ,_countdown(5)
{
    ...
    //连接信号和槽
    connect(_countdown_timer,&QTimer::timeout,[this](){//定时器的超时信号timeout连接下面的槽函数[this]
        if(_countdown==0){
...
void RegisterDialog::slot_reg_mod_finish(ReqId id, QString res, ErrorCodes err)
{
    ...
    //转json对象
    _handlers[id](jsonDoc.object());//验证，判断
    return ;
}
...
void RegisterDialog::initHttpHandlers()
{
    ...
    _handlers.insert(ReqId::ID_REG_USER, [this](QJsonObject jsonObj){
        ...
        qDebug()<<"user uuid is "<<jsonObj["uuid"].isString();
        qDebug()<< "email is " << email ;
        ChangeTipPage();
    });
}
